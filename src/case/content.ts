...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'the cuts land on the proportions nature and craft keep reusing: halves, thirds, the golden section that sets the seed spiral of a sunflower, and the silver section of classical japanese carpentry. and beyond free growth, the engine knows the cut programs randomness can never wander into. it can whirl: the spiral of the nautilus, each turn slicing a slab and rotating a quarter, tightening into an eye. it can mirror: one half grown, the other its exact reflection, the bilateral symmetry of nearly everything that walks. it can echo: the same split repeated inside itself at falling scales, the way a fern frond repeats its own outline. each is a deliberate minority genre; when the spiral first learned to win it took a third of all seeds, and the cure, as ever, was measuring and rebalancing until every style could win on merit.',
